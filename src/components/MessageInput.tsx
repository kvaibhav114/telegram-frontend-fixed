...
export function MessageInput({ onSend, onTyping, replyTo, onCancelReply, editingMsg, onCancelEdit, onConfirmEdit }: Props) {
  const [text, setText] = useState("");
  const [emojiOpen, setEmojiOpen] = useState(false);
  const ref = useRef<HTMLTextAreaElement>(null);

  // When editing starts, populate text
  useEffect(() => {
    if (editingMsg) { setText(editingMsg.content); ref.current?.focus(); }
  }, [editingMsg]);
      ...
      {(replyTo || editingMsg) && (
        <div className="flex items-center gap-2 px-4 pt-2">
          <div className="flex-1 flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary/10 border-l-2 border-primary text-xs">
            {editingMsg ? <Pencil className="size-3 text-primary shrink-0" /> : null}
            <div className="min-w-0">
              <div className="font-medium text-primary">{editingMsg ? "Editing message" : `Reply to ${replyTo!.senderName}`}</div>
              <div className="text-muted-foreground truncate">{(editingMsg ?? replyTo)!.content}</div>
            </div>
          </div>
          <button onClick={editingMsg ? onCancelEdit : onCancelReply} className="size-7 grid place-items-center rounded-md hover:bg-accent">
      ...
      <div className="p-2.5 md:p-3">
        {emojiOpen && (
          <div className="absolute bottom-20 left-4 z-30">
            <EmojiPickerPopover onSelect={(e) => setText((t) => t + e)} onClose={() => setEmojiOpen(false)} />
          </div>
        )}
        <div className="flex items-end gap-1.5 bg-card rounded-2xl px-1.5 py-1 border border-border focus-within:border-primary/50 transition">
          ...
          <textarea ref={ref} value={text}
            onChange={(e) => { setText(e.target.value); onTyping?.(e.target.value.trim().length > 0); }}
